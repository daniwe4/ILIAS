	...
	public function fillRow($set) {
		...
		foreach ($this->order as $column_id) {
			if(isset($relevant[$column_id])) {
				...
				} elseif(strpos($column_id,'standard_user_data_presentation_') === 0) {
					$this->tpl->setCurrentBlock('usr_data_field');
					$this->tpl->setVariable('VAL_USRDATAFIELD', (string)$set[$column_id]);
				} else {
