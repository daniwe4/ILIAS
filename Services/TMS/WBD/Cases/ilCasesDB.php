...
use CaT\WBD\Cases;
use CaT\WBD\ErrorLog\Save as WBD_ERROR_LOG;
...
>>>>>>> b82a6042d5... TMS 4281 change sql for report participation. check existing wbd id and resolved or no errors
        \ilDBInterface $db,
...
<<<<<<< HEAD
    ...
    public function getParticipationsToReport(int $gutberaten_udf_id, int $announce_wbd_id) : array
    {
        ...
        $wbd_booking_id = $space->table(self::TABLE_ANNOUNCEMENT_STATUS)->field(self::FIELD_WBD_BOOKING_ID);
        $wbd_announcement_status = $space->table(self::TABLE_ANNOUNCEMENT_STATUS)->field(self::FIELD_WBD_ANNOUNCEMENT_STATUS);
        $space
            ->addFilter($this->pre_f->_ANY(
                $wbd_booking_id->IS_NULL(),
                $wbd_booking_id->EQ($this->pre_f->str(''))
            ))->addFilter($this->pre_f->_ANY(
                $wbd_announcement_status->IS_NULL(),
                $wbd_announcement_status->IN($this->pre_f->list_string_by_array([self::WBDA_STATUS_RETRY,'']))
            ))->addFilter($space->table(self::TABLE_COURSE)->field(self::FIELD_CRS_ID)->GT($this->pre_f->int(0)));
    ...
    public function getParticipationsToCancel() : array
    {
        $q = "SELECT wbd.crs_id, wbd.usr_id, wbd.gutberaten_id, wbd.wbd_booking_id," . PHP_EOL
            ...
            . " WHERE hcrs.title != wbd.title " . PHP_EOL
            ...
            . "     OR IF((hcrs.end_date IS NOT NULL AND hcrs.end_date != '0001-01-01'), hcrs.end_date, husrcrs.ps_acquired_date) != wbd.end_date"
        ;
    ...
    protected function participationsSpace(int $gutberaten_udf_id, int $announce_wbd_id) : TableRelations\Tables\TableSpace
    {
        ...
        $p_bookin_status =
            $this->tab_f->Table(self::TABLE_ANNOUNCEMENT_STATUS, self::TABLE_ANNOUNCEMENT_STATUS)
                ->addField($this->tab_f->field(self::FIELD_CRS_ID))
                ->addField($this->tab_f->field(self::FIELD_USR_ID))
        ...
        $participation_space
            ...
            ->addTableSecondary($p_bookin_status)
            ->addDependency(
                $this->tab_f->TableJoin(
            ...
            ->addDependency(
                $this->tab_f->TableLeftJoin(
                    ...
                    $p_bookin_status,
                    $this->pre_f->_ALL(
                        $participations->field(self::FIELD_CRS_ID)->EQ($p_bookin_status->field(self::FIELD_CRS_ID)),
                        $participations->field(self::FIELD_USR_ID)->EQ($p_bookin_status->field(self::FIELD_USR_ID))
                    )
                )
            )
            ->addFilter(
...
=======
	...
	{
		$space = $this->participationsSpace($gutberaten_udf_id, $announce_wbd_id);
		$error_status = $space->table(self::TABLE_ERROR_LOG)->field(self::FIELD_ERROR_STATUS);
		$space
			->addFilter(
				$this->pre_f->_ANY(
					$error_status->IS_NULL(),
					$error_status->EQ($this->pre_f->str('resolved'))
				)
			)
		;
	...
	protected function participationsSpace(int $gutberaten_udf_id, int $announce_wbd_id) : TableRelations\Tables\TableSpace
	{
		...
		$p_error_status =
			$this->tab_f->Table(self::TABLE_ERROR_LOG, self::TABLE_ERROR_LOG)
				->addField($this->tab_f->field(self::FIELD_CRS_ID))
				->addField($this->tab_f->field(self::FIELD_USR_ID))
		...
		$participation_space
			...
			->addTablePrimary($courses)
			->addTablePrimary($wbd_crs)
			->addTableSecondary($p_error_status)
			->addDependency(
				$this->tab_f->TableJoin(
					$participation_space->table(self::TABLE_USR_DATA),
					$participations,
			...
			->addDependency(
				$this->tab_f->TableLeftJoin(
					...
					$p_error_status,
					$this->pre_f->_ALL(
						$participations->field(self::FIELD_CRS_ID)->EQ($p_error_status->field(self::FIELD_CRS_ID)),
						$participations->field(self::FIELD_USR_ID)->EQ($p_error_status->field(self::FIELD_USR_ID))
					)
				)
			)
			->addFilter(
