...
class TMSPositionHelper
{
                ...
                case ilOrgUnitAuthority::OVER_EVERYONE:
                    switch ($authority->getScope()) {
                        ...
                        case ilOrgUnitAuthority::SCOPE_SINGLE_SUBSEQUENT_ORGU:
                            $ids = array_merge(
                                $ids,
                                $this->orgua_queries->getUserIdsOfOrgUnitsOfUsersPosition($position->getId(), $user_id, false, true)
                            );
                            break;
                    }
                    break;
                default:
                    switch ($authority->getScope()) {
                        ...
                        case ilOrgUnitAuthority::SCOPE_SINGLE_SUBSEQUENT_ORGU:
                            $ids = array_merge(
                                $ids,
                                $this->orgua_queries->getUserIdsOfUsersOrgUnitsInPosition($user_id, $position->getId(), $authority->getOver(), false, true)
                            );
                            break;
                    }
            }
            ...
            foreach ($authorities as $akey => $authority) {
                ...
                if ((int) $authority->getScope() === \ilOrgUnitAuthority::SCOPE_SUBSEQUENT_ORGUS) {
                    ...
                    foreach ($orgus as $orgu_ref_id) {
                        $lower_orgus = array_filter(
                            $orgu_tree->getAllChildren($orgu_ref_id),
                ...
                if ((int) $authority->getScope() === \ilOrgUnitAuthority::SCOPE_SINGLE_SUBSEQUENT_ORGU) {
                    $subsequent_orgus = [];
                    foreach ($orgus as $orgu_ref_id) {
                        $subsequent_orgus = array_merge($subsequent_orgus, $orgu_tree->getChildren($orgu_ref_id));
                    }
                    if (count($subsequent_orgus) === 0) {
                        continue;
                    }
    ...
    protected function getFirstSubsequentOrgus(int $orgu_id) : array
    {
        return \ilObjOrgUnitTree::_getInstance()->getChildren($orgu_id);
    }
