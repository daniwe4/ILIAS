...
class UnboundCourseProvider extends SeparatedUnboundProvider
{
    ...
    protected function getSessionAppointments(Entity $entity, $object)
    {
        ...
        if (count($sessions) > 0) {
            foreach ($sessions as $session) {
                ...
                $end_date = $this->formatDate($appointment->getEnd());
    ...
    protected function getAppointmentOutput(array $appointments) : array
    {
        ...
        $presentation_dates = array_map(
            function ($times) {
                $date = $times["date"];
                if ($times["date"] != $times["end_date"]) {
                    $date .= " bis " . $times["end_date"];
                }
                return $date
                . ": " . $times["start_time"]
                . " - " . $times["end_time"]
    ...
    protected function formatPeriod(\ilDateTime $date1, \ilDateTime $date2) : string
    {
        ...
        if (ilPluginAdmin::isPluginActive('venues')) {
            ...
            if ($custom_assignment) {
                ...
                $ret[] = $this->createCourseInfoObject(
                    $entity,
                    $txt("venue"),
                    join("<br />", $venue) . "<br />" . $postcode . " " . $city,
                    300,
                    [
                    CourseInfo::CONTEXT_TEP_SESSION_DETAILS
                  ]
