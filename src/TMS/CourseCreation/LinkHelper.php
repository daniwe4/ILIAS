    ...
    */
    ...
    protected function getCourseTemplateSelectionModal(\ILIAS\UI\Factory $ui_factory, array $info, array $parent_guis, $parent_cmd, $parent_ref_id)
    {
        ...
        return $ui_factory->modal()
            ->roundtrip(
                $lng->txt("choose_course_template"),
    ...
    protected function maybeShowRequestInfo(\ilCourseCreationPlugin $xccr_plugin = null, $waiting_time = 30000)
    {
        $ret = false;
        $usr = $this->getUser();
        $requests = $this->getUsersDueRequests($usr, $xccr_plugin);
        if (count($requests) === 0) {
            return $ret;
        }

        $xccl = $this->getCourseCreationPlugin();
        if (is_null($xccl)) {
            return $ret;
        }
        ...
        if (
            ...
            $message = $this->getMessage($requests, "course_creation_multi_message");
        } else {
            $message = $this->getMessage($requests, "course_creation_message", $waiting_time);
            $ret = true;
        }
     ...
     * @return string
     */
    protected function getMessage(array $requests, $header, $waiting_time = null)
    {
        ...
        $tpl->setVariable(
            "HEADER",
            $this->getLng()->txt($header)
        );
        foreach ($requests as $key => $request) {
            $tpl->setCurrentBlock("request_title");
            $tpl->setVariable("TITLE", $this->getTrainingTitleByRequest($request));
            $tpl->parseCurrentBlock();
        }

        if (!is_null($waiting_time)) {
            $tpl->setCurrentBlock("reload");
    ...
    protected function canCreateRequest(
        \ilObjUser $user,
        \RbacImpl $rbac,
        \ilCourseCreationPlugin $xccr_plugin
    ) : bool {
        $actions = $xccr_plugin->getActions();
        $role_ids = $actions->getRoleIdsForMultiplyRequestCreation();
        if (
            count($role_ids) != 0 &&
            count(
                array_intersect(
                    $rbac->getAssignedGlobalRolesOf($user->getId()),
                    $role_ids
                )
            ) > 0
        ) {
        ...
        return count($this->getUsersDueRequests($user, $xccr_plugin)) == 0;
    }
}
