...
class ilCourseTemplateDB implements CourseTemplateDB
{
    ...
    public function getCreatableCourseTemplates(int $user_id)
    {
        ...
        foreach ($this->getAllCourseTemplates() as $ref_id => $info) {
            if (!self::_mayUserCreateCourseFromTemplate($user_id, $ref_id)) {
                continue;
            }

            $type = $info->getTypeTitle();
    ...
    public function getAllCourseTemplates()
    {
        ...
        foreach ($copy_setting_nodes as $cs_node) {
            ...
            while ($p = array_shift($path)) {
                if ($p["type"] == "crs") {
                    $crs_ref_id = (int) $p["child"];
                    $crs_template_info[$crs_ref_id] = new CourseTemplateInfo(
                        $this->purgeTemplateInTitle($p["title"]),
                        $crs_ref_id,
                        $this->getCategoryTitle($path),
                        $this->getCourseType($crs_ref_id)
                    );
                }
            }
        }
