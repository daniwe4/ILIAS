...
use CaT\Plugins\TrainingSearch\DI;

/**
 * Plugin base class. Keeps all information the plugin needs
 */
class ilTrainingSearchPlugin extends ilRepositoryObjectPlugin
{
    use DI;

    const COPY_OPERATION_ID = 58;
    const TTL_FOR_RESULTS_IN_APC_IN_S = 300;
    ...
    protected function createPluginPermissions($type_id, \ilDBInterface $db)
    {
        ...
        $new_rbac_options = array(
            array("use_search", "Use this search", "object", 5000)
        );
    ...
    public function getAllSearchObjectsInfos() : array
    {
        $q = 'SELECT od.title, oref.ref_id' . PHP_EOL
            . ' FROM object_data od' . PHP_EOL
            . ' JOIN object_reference oref' . PHP_EOL
            . '     ON oref.obj_id = od.obj_id' . PHP_EOL
            . ' WHERE od.type = "xtrs"' . PHP_EOL
    ...
    public function getDI()
    {
        if (is_null($this->di)) {
            global $DIC;
            $this->di = $this->getPluginDIC($this, $DIC);
        }
