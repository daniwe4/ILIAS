...
use CaT\Plugins\Accomodation\Mailing\ilMailContextOACReminder;
...
class UnboundGlobalProvider extends SeparatedUnboundProvider
{
    ...
    public function buildComponentsOf($component_type, Entity $entity)
    {
        if ($component_type === Mailing\MailContext::class) {
            return [
                new MailContextAccomodations($entity, $this->owner()),
                new ilMailContextOACReminder($entity, $this->owner())
            ];
        }
    ...
    public static function createGlobalProvider()
    {
        ...
        if (!$provider) {
            ...
            $provider_db->createSeparatedUnboundProvider(
                ...
                'Customizing/global/plugins/Services/Repository/RepositoryObject/Accomodation/classes/UnboundGlobalProvider.php'
            );
        }
    }
