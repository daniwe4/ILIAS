...
class Report
{
    ...
    public function __construct(
        \ilTrainingDemandRetardedPlugin $plugin,
        \ilDBInterface $db,
    ...
    protected function actionMenuFor($ref_id)
    {
        ...
        foreach ($this->action_links->withRefId($ref_id)->getAdministratedTrainingActionLinks() as $title => $link) {
            $l->addItem(
                $title,
                $title,
                $link,
    ...
    {
        if (array_key_exists('sum_idd', $row)) {
            $row['sum_idd'] = $this->minutesToTimeString((int) $row['sum_idd']);
        }
        ...
        if (array_key_exists('costcenter_finalized', $row)) {
            if ((int) $row['costcenter_finalized'] === 1) {
                $row['costcenter_finalized'] = 'Ja';
            } else {
                $row['costcenter_finalized'] = 'Nein';
            }
        }
        if (array_key_exists('participation_finalized_date', $row) && $row['participation_finalized_date'] > '0001-01-01') {
            $date = \DateTime::createFromFormat('Y-m-d', $row['participation_finalized_date'])->format('d.m.Y');
            $row['participation_finalized_date'] = $date;
        } else {
            $row['participation_finalized_date'] = '';
        }
        ...
        if (array_key_exists('nights_resolved_count', $row)
            && !is_null($row['nights_resolved_count'])
        ) {
        ...
        if (array_key_exists('net_total_cost', $row)) {
            $row["net_total_cost"] = number_format((float) $row["net_total_cost"], 2, ",", ".");
        }

        if (array_key_exists('gross_total_cost', $row)) {
            $row["gross_total_cost"] = number_format((float) $row["gross_total_cost"], 2, ",", ".");
        }
    ...
    public function applyFilterToSpace(array $settings)
    {
        ...
        $this->maybeApplyCrsEndFilter($settings['period_start'], $settings['period_end']);
    ...
    protected function maybeApplyCrsEndFilter($period_start, $period_end)
    {
        if (!$period_start && !$period_end) {
            return;
        }
        $end = $this->space()->table('crs')->field('end_date');
        if ($period_start && $period_end) {
            $filter = $end->GE()->date($period_start)
                ->_AND($end->LE()->date($period_end));
        }
        if ($period_start && !$period_end) {
            $filter = $end->GE()->date($period_start);
        }
        if (!$period_start && $period_end) {
            $filter = $end->LE()->date($period_end);
        }
        $filter = $filter->_OR($end->IS_NULL());
    ...
    protected function buildSpace()
    {
        ...
        $crs_data = $crs_data->addField($this->tf->field('max_members'))
                        ...
                        ->addField($this->tf->field('costcenter_finalized'))
                        ->addField($this->tf->field('participation_finalized_date'))
                        ->addField($this->tf->field('net_total_cost'))
                        ->addField($this->tf->field('gross_total_cost'))
                        ;
        ...
        if ($this->isEduTrackingActive()) {
            $bookings_src = $bookings_src->addField($this->tf->field('idd_learning_time'));
        }
        ...
        $booking_crs_idd = $this->tf->Table(self::HEAD_COURSE_TABLE, 'crs_idd')
                        ->addField($this->tf->field('crs_id'))
                        ->addField($this->tf->field('idd_learning_time'));
        $bookings_space = $this->tf->TableSpace()
                        ->addTablePrimary($bookings_src)
                        ->addTablePrimary($booking_crs_idd)
                        ->setRootTable($bookings_src)
                        ->addDependency(
                            $this->tf->TableJoin(
                                $bookings_src,
                                $booking_crs_idd,
                                $bookings_src->field('crs_id')
                                    ->EQ($booking_crs_idd->field('crs_id'))
                            )
                        )
                        ...
                        ->request($this->tf->sum(
                            ...
                            $this->tf->ifThenElse(
                                '',
                                $bookings_src->field('booking_status')->EQ($this->pf->str('participant'))
                                    ->_AND($bookings_src->field('participation_status')->EQ($this->pf->str('successful'))),
                                $this->tf->constInt('', 1),
                                $this->tf->constInt('', 0)
                            )
                        ))
                        ->request($this->tf->sum(
                            ...
                            $this->tf->ifThenElse(
                                '',
                                $bookings_src->field('booking_status')->EQ($this->pf->str('participant'))
                                    ->_AND($bookings_src->field('participation_status')->EQ($this->pf->str('absent'))),
                                $this->tf->constInt('', 1),
                                $this->tf->constInt('', 0)
                            )
                        ))
                        ->request($this->tf->sum(
                            ...
                            $this->tf->ifThenElse(
                                '',
                                $bookings_src->field('booking_status')->EQ($this->pf->str('waiting_cancelled')),
                                $this->tf->constInt('', 1),
                                $this->tf->constInt('', 0)
                            )
                        ))
                        ->request($this->tf->sum(
                            ...
                            $this->tf->ifThenElse(
                                '',
                                $bookings_src->field('booking_status')->EQ($this->pf->str('cancelled_after_deadline')),
                                $this->tf->constInt('', 1),
                                $this->tf->constInt('', 0)
                            )
                        ));
        if ($this->isEduTrackingActive()) {
            $bookings_space = $bookings_space->request($this->tf->sum(
                'sum_idd',
                $this->tf->ifThenElse(
                                '',
                                $bookings_src->field('booking_status')
                                    ->EQ($this->pf->str('participant'))
                                    ->_AND(
                                        $bookings_src->field('participation_status')
                                            ->EQ($this->pf->str('successful'))
                                    ),
                                $this->tf->ifThenElse(
                                    '',
                                    $bookings_src->field('idd_learning_time')->IS_NULL()->_NOT(),
                                    $bookings_src->field('idd_learning_time'),
                                    $booking_crs_idd->field('idd_learning_time')
                                ),
                                $this->tf->constInt('', 0)
                            )
                        ));
        ...
        $bookings_space = $bookings_space->request($this->tf->countAll('cnt_members'))
        ...
        $space = $this->tf->TableSpace()
            ...
            ->addDependency(
                $this->tf->TableLeftJoin(
                    $crs_data,
                    $object_reference,
                    $object_reference->field('obj_id')->EQ($crs_data->field('crs_id'))
                        ->_AND($object_reference->field('deleted')->IS_NULL())
                )
            )
            ->addFilter($crs_data->field('end_date')->LT()->date(new \DateTime()))
        ;
    ...
    public function configureTable(\SelectableReportTableGUI $table)
    {
        ...
        $table = $table
            ...
            ->defineFieldColumn(
                ...
                'cnt_booked',
                ['cnt_booked' => $space->table('bookings')->field('cnt_booked')],
                true
            )
            ->defineFieldColumn(
                $this->plugin->txt('cnt_participated'),
                'cnt_participated',
                ['cnt_participated' => $space->table('bookings')->field('cnt_participated')],
                true
            )
            ->defineFieldColumn(
                $this->plugin->txt('cnt_absent'),
                'cnt_absent',
                ['cnt_absent' => $space->table('bookings')->field('cnt_absent')],
                true
            )
            ->defineFieldColumn(
                $this->plugin->txt('cnt_cancelled_waiting'),
                'cnt_cancelled_waiting',
                ['cnt_cancelled_waiting' => $space->table('bookings')->field('cnt_cancelled_waiting')],
                true
            )
            ->defineFieldColumn(
                $this->plugin->txt('cnt_cancelled_after_dl'),
                'cnt_cancelled_after_dl',
                ['cnt_cancelled_after_dl' => $space->table('bookings')->field('cnt_cancelled_after_dl')],
                true
            )
            ->defineFieldColumn(
                $this->plugin->txt('participation_finalized_date'),
                'participation_finalized_date',
                ['participation_finalized_date' => $space->table('crs')->field('participation_finalized_date')],
                true

            )
            ->defineFieldColumn(
                $this->plugin->txt('net_total_cost'),
                'net_total_cost',
                ['net_total_cost' => $space->table('crs')->field('net_total_cost')],
                true

            )
            ->defineFieldColumn(
                $this->plugin->txt('gross_total_cost'),
                'gross_total_cost',
                ['gross_total_cost' => $space->table('crs')->field('gross_total_cost')],
                true

            )
            ->defineFieldColumn(
                $this->plugin->txt('costcenter_finalized'),
                'costcenter_finalized',
                ['costcenter_finalized' => $space->table('crs')->field('costcenter_finalized')],
                true
        ...
        if ($this->isEduTrackingActive()) {
            $table = $table->defineFieldColumn(
                $this->plugin->txt('sum_idd'),
                'sum_idd',
                ['sum_idd' => $space->table('bookings')->field('sum_idd')],
                true,
                true
            );
        }
        ...
        $table->setDefaultOrderColumn('crs_date', \SelectableReportTableGUI::ORDER_DESC);
