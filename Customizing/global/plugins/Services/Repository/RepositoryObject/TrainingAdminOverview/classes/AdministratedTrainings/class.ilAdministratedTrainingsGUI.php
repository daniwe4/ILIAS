...
/**
 * Contaner gui to show administrated trainings
 *
 * @author Stefan Hecken 	<stefan.hecken@concepts-and-training.de>
 */
class ilAdministratedTrainingsGUI
{
    ...
    /**
     * @var ilObjTrainingAdminOverviewGUI
     */
    protected $parent;
    ...
    public function __construct(ilObjTrainingAdminOverviewGUI $parent, TrainingAdminOverview\ilObjActions $actions, CourseTemplateDB $crs_template_db)
    {
        global $DIC;
    ...
    /**
     * Show all current trainings where user is admin
     *
     * @return void
     */
    protected function showAssignments()
    {
        ...
        $filter = array();
        $data = $this->actions->getAdministratedTrainingsFor((int) $this->g_user->getId(), $filter);
     ...
     * Render table
     *
     * @param TrainingAdminOverview\AdministratedTrainings\AdministratedTraining[] 	$data
     *
     * @return void
     */
    protected function showTable(array $data)
    {
        ...
        if (count($course_templates) > 0 &&
            ...
            $this->addCourseTemplateSelectionModalToToolbar(
                ...
                $this->g_toolbar,
                $course_templates,
                ["ilObjPluginDispatchGUI","ilObjTrainingAdminOverviewGUI","ilAdministratedTrainingsGUI"],
                "showAssignments",
                $this->parent->getObjectRefId()
            );
        }

        $table = new TrainingAdminOverview\AdministratedTrainings\ilAdministratedTrainingsTableGUI($this);
        $table->setData($data);
    ...
    protected function getPeriodSortingClosure($direction)
    {
        if ($direction == "asc") {
            return function ($a, $b) {
                if (is_null($a->getCourseStart()) && is_null($b->getCourseStart())) {
                    return 0;
                } elseif (is_null($a->getCourseStart()) && !is_null($b->getCourseStart())) {
                    return 1;
                } elseif (!is_null($a->getCourseStart()) && is_null($b->getCourseStart())) {
                    return -1;
                }

                $start_date_a = $a->getCourseStart()->get(IL_CAL_DATE);
                $start_date_b = $b->getCourseStart()->get(IL_CAL_DATE);
