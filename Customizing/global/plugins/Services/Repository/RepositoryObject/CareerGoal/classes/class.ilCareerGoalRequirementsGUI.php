...
class ilCareerGoalRequirementsGUI
{
	...
	{
		$to_delete_obj_id = (int)$_GET["obj_id"];
	...
	{
		$to_delete_obj_id = (int)$_POST["obj_id"];
	...
	protected function confirmedDeleteSelected()
	{
		...
		foreach ($to_delete_obj_ids as $value) {
			$ret[] = $this->getActions()->deleteRequirement((int)$value);
		}
	...
	{
		$this->gTabs->addSubTab(self::CMD_SHOW, $this->txt("show_requirement"), $this->gCtrl->getLinkTarget($this, self::CMD_SHOW));

		$this->gTabs->addSubTab(self::CMD_SORT, $this->txt("sort_requirement"), $this->gCtrl->getLinkTarget($this, self::CMD_SORT));
