...
class ilCareerGoalObservationsTableGUI extends \ilTable2GUI {
	use ilFormHelper;
	...
	public function __construct($a_parent_obj, $a_parent_cmd = "", $a_template_context = "", $sort = false) {
		...
		$this->setId("career_goal_observations");
		...
		if(!$sort) {
			...
			$this->setTitle($this->txt("observation_table_sort_title"));
		}

		$this->addColumn($this->txt("title"), null);
		$this->addColumn($this->txt("description"), null);
		$this->addColumn($this->txt("requirements"), null);
	...
	protected function fillRow($row) {
		...
		$this->tpl->setVariable("REQUIREMENTS", $row["requirements"]);
	...
	protected function getMultiCommands() {
		return array($this->possible_cmd["CMD_DELETE_SELECTED_OBSERVATIONS"] => self::CAPTION_DELETE_SELECTED_OBSERVATIONS);
	}
	...
	protected function getActionMenuItems($obj_id) {
		...
		$items = array();
		$items[] = array("title" => $this->txt("edit_observation"), "link" => $link_edit, "image" => "", "frame"=>"");
		$items[] = array("title" => $this->txt("delete_observation"), "link" => $link_delete, "image" => "", "frame"=>"");
