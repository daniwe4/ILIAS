...
class ilTMSUserCertificateRepository extends \ilUserCertificateRepository
{
    ...
    public function __construct(
        ...
        parent::__construct($this->database, $this->logger, $this->defaultTitle);
    }
...
AND il_cert_user_cert.obj_type = ' . $this->database->quote(self::OBJECT_TYPE, 'text') . '
...
WHERE obj_id = ' . $this->database->quote($objectId, 'integer') . '
 AND currently_active = ' . $this->database->quote(1, 'integer') . '
     ...
     * @return array
     */
    protected function fetchCertificatesOfObject(int $objId, int $userId) : array
    {
...
AND obj_type = ' . $this->database->quote(self::OBJECT_TYPE, 'text')
     ...
     * @return string
     */
    protected function fetchLatestVersion(int $objId, int $userId) : string
    {
        $this->logger->info(sprintf(
     ...
     * @throws \ilDatabaseException
     */
    protected function deactivatePreviousCertificates(int $objId, int $userId)
    {
...
;
     ...
     * @return \ilUserCertificate
     */
    protected function createUserCertificate($row) : \ilUserCertificate
    {
