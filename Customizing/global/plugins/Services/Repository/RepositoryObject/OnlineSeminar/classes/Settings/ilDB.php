...
class ilDB implements DB
{
     ...
     * @inhertidoc
     */
    public function update(OnlineSeminar $online_seminar)
    {
        $start = $online_seminar->getBeginning();
        if ($start !== null) {
            $start = $start->get(IL_CAL_DATETIME);
        }

        $end = $online_seminar->getEnding();
        if ($end !== null) {
            $end = $end->get(IL_CAL_DATETIME);
        }

        $where = array("obj_id" => array("integer", $online_seminar->getObjId()));

        $values = array("beginning" => array("string", $start),
            "vc_type" => array("string", $online_seminar->getVCType()),
            "ending" => array("string", $end),
            "admission" => array("string", $online_seminar->getAdmission()),
            "url" => array("string", $online_seminar->getUrl()),
            "is_online" => array("integer", $online_seminar->getOnline()),
            "lp_mode" => array("integer", $online_seminar->getLPMode()),
            "is_finished" => array("integer", $online_seminar->isFinished())
        );
    ...
    public function create(
        ...
        $values = array("obj_id" => array("integer", $online_seminar->getObjId()),
            "vc_type" => array("string", $online_seminar->getVCType()),
