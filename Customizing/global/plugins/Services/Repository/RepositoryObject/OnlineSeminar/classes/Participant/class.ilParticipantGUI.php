...
class ilParticipantGUI
{
    ...
    public function throwFinalizedEvent()
    {
        ...
        $this->g_event_handler->raise("Plugin/OnlineSeminar", "online_seminar_finalized", $payload);
    }
    ...
    {
        $this->g_log->write('OnlineSeminar, error reading file: ' . $message);
        $file_storage = $this->actions->getObject()->getFileStorage();
    ...
    /**
     * User books himself to online seminar
     *
     * @return null
     */
    protected function selfBookUser()
    {
        ...
        $link = $this->g_ctrl->getLinkTargetByClass("ilObjOnlineSeminarGUI", "", "", false, false);
    ...
    protected function bookCourseMember()
    {
        ...
        if ($this->actions->getObject()->getSettings()->isFinished()) {
            \ilUtil::sendInfo($this->txt("online_seminar_finished"), true);
            $this->g_ctrl->redirect($this, self::CMD_SHOW_PARTICIPANTS);
        }
    ...
    /**
     * Imports course member to online seminar
     *
    ...
    /**
     * Delete all online seminar participant not member of course
     *
