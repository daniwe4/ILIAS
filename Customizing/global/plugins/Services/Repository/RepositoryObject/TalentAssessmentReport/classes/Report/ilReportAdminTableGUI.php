...
class ilReportAdminTableGUI extends \ilTable2GUI
{
	public function __construct($a_parent_obj, \Closure $txt, ilActions $actions, $apply_filter = false, $rest_filter = false, $a_parent_cmd = "", $a_template_context = "")
	{
		...
		$this->setTitle($this->txt("all_observations"));
		$this->setDescription($this->txt("all_observations_info"));
	...
	public function initFilter()
	{
		...
		$item = new \ilMultiSelectInputGUI($this->txt("career_goal"), "career_goal");
		...
		$item = new \ilMultiSelectInputGUI($this->txt("observer"), "observer");
		$item->setOptions($this->parent_obj->getObserverOptions());
		$this->addFilterItem($item);
	}
