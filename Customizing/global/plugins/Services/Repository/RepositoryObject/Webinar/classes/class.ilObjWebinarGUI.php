...
require_once(__DIR__ . "/Settings/class.ilWebinarGUI.php");
 ...
 *
 * @ilCtrl_isCalledBy ilObjWebinarGUI: ilRepositoryGUI, ilAdministrationGUI, ilObjPluginDispatchGUI
 * @ilCtrl_Calls ilObjWebinarGUI: ilPermissionGUI, ilInfoScreenGUI, ilObjectCopyGUI, ilCommonActionDispatcherGUI
 * @ilCtrl_Calls ilObjWebinarGUI: ilWebinarGUI, ilParticipantGUI, ilRepositorySearchGUI, ilLPSettingsGUI, ilExportGUI
 */
class ilObjWebinarGUI extends ilObjectPluginGUI
{
    ...
    public function performCommand($cmd)
    {
        ...
        switch ($next_class) {
            case "ilrepositorysearchgui":
                ...
                break;
            case "ilwebinargui":
                $this->activateTab(ilWebinarGUI::CMD_EDIT_PROPERTIES);
                $vc_form_helper = $this->object->getVCFormHelper();
                $gui = new ilWebinarGUI($this->object->getActions(), $this->vc_actions, $vc_form_helper);
            ...
            default:
                switch ($cmd) {
                    case ilWebinarGUI::CMD_EDIT_PROPERTIES:
                        $this->redirectWebinar();
                        break;
    ...
    */
    ...
    {
        return ilWebinarGUI::CMD_EDIT_PROPERTIES;
    }
    ...
    */
    ...
    public function afterSave(\ilObject $newObj)
    {
        ...
        $fnc = function (CaT\Plugins\Webinar\Settings\Webinar $s) use ($post) {
    ...
    /**
     * Redirect to webinar gui
     *
     * @return null
     */
    protected function redirectWebinar()
    {
        $link = $this->ctrl->getLinkTargetByClass(
            array("ilObjWebinarGUI", "ilWebinarGUI"),
            ilWebinarGUI::CMD_EDIT_PROPERTIES,
            "",
            false,
            false
        );
    ...
    {
        $link = $this->ctrl->getLinkTargetByClass(
            array("ilObjWebinarGUI", "ilParticipantGUI"),
    ...
    protected function setTabs()
    {
        ...
        if ($this->access_handler->checkAccess("edit_member", "", $this->object->getRefId())) {
            $participant = $this->ctrl->getLinkTargetByClass(array("ilObjWebinarGUI", "ilParticipantGUI"), ilParticipantGUI::CMD_SHOW_PARTICIPANTS);
            $this->tabs_gui->addTab(ilParticipantGUI::CMD_SHOW_PARTICIPANTS, $this->txt("tab_member"), $participant);
        }

        if ($this->access_handler->checkAccess("write", "", $this->object->getRefId())) {
            $settings = $this->ctrl->getLinkTargetByClass(array("ilObjWebinarGUI", "ilWebinarGUI"), ilWebinarGUI::CMD_EDIT_PROPERTIES);
            $this->tabs_gui->addTab(ilWebinarGUI::CMD_EDIT_PROPERTIES, $this->txt("tab_settings"), $settings);
        }

        if ($this->access_handler->checkAccess("edit_participation", "", $this->object->getRefId())) {
            $lp = $this->ctrl->getLinkTargetByClass(array("ilObjWebinarGUI", "ilLPSettingsGUI"), ilLPSettingsGUI::CMD_LP);
    ...
    public function addInfoItems($info)
    {
        ...
        if ($booked) {
            ...
            if ($settings->getAdmission() == "self_booking") {
                ...
                $link = $this->ctrl->getLinkTargetByClass(array("ilObjWebinarGUI", "ilParticipantGUI"), ilParticipantGUI::CMD_SELF_BOOK_USER);
