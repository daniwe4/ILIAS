...
use \CaT\Plugins\RoomSetup\ilObjectActions;

/**
 * Class for the settings of an repo object
 */
class ilRoomSetupSettingsGUI
{
    ...
    /**
     * @var \ilObjRoomSetupGUI
     */
    protected $parent_gui;

    public function __construct(\ilObjRoomSetupGUI $parent_gui, ilObjectActions $object_actions, \Closure $txt)
    {
        global $DIC;
    ...
    protected function initForm()
    {
        ...
        //service:
        $rg = new \ilRadioGroupInputGUI($this->txt("recipient_mode_service"), ilObjectActions::F_SERVICE_RECIPIENT_MODE);
        ...
        $ti = new \ilEMailInputGUI($this->txt("recipient"), ilObjectActions::F_SERVICE_RECIPIENT);
        $ti->setInfo($this->txt("recipient_info"));
        $ti->setSize(40);
        $ti->setRequired(true);
        $opt->addSubItem($ti);
        $ni = new \ilNumberInputGUI($this->txt("send_days_before"), ilObjectActions::F_SERVICE_SEND_DAYS_BEFORE);
        $ni->setMinValue(0, true);
        $ni->setInfo($this->txt("send_days_before_info_service"));
        $ni->setRequired(true);
        $opt->addSubItem($ni);
        $rg->addOption($opt);
        ...
        //setup:
        $rg = new \ilRadioGroupInputGUI($this->txt("recipient_mode_room"), ilObjectActions::F_SETUP_RECIPIENT_MODE);
        $opt = new \ilRadioOption($this->txt('course_venue'), ilObjectActions::M_COURSE_VENUE);
        $rg->addOption($opt);
        $opt = new \ilRadioOption($this->txt('selection'), ilObjectActions::M_SELECTION);
        $ti = new \ilEMailInputGUI($this->txt("recipient"), ilObjectActions::F_SETUP_RECIPIENT);
        $ti->setInfo($this->txt("recipient_info"));
        $ti->setSize(40);
        $ti->setRequired(true);
        $opt->addSubItem($ti);
        $ni = new \ilNumberInputGUI($this->txt("send_days_before"), ilObjectActions::F_SETUP_SEND_DAYS_BEFORE);
        $ni->setMinValue(0, true);
        $ni->setInfo($this->txt("send_days_before_info_room"));
        $ni->setRequired(true);
        $opt->addSubItem($ni);
        $rg->addOption($opt);
