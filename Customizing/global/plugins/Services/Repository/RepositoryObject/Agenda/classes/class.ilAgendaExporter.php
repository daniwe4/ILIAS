...
class ilAgendaExporter extends ilXmlExporter
{
    ...
    public function getXmlRepresentation($a_entity, $a_schema_version, $a_id)
    {
        ...
        $obj = new ilObjAgenda((int) $ref_id);
        $settings = $obj->getSettings();

        $writer = new ilXmlWriter();
        $writer->xmlStartTag("xage");
        $writer->xmlElement("title", null, $obj->getTitle());
        $writer->xmlElement("description", null, $obj->getDescription());
        $writer->xmlElement("start_time", null, $settings->getStartTime()->format("H:i"));
        $writer->xmlEndTag("xage");
