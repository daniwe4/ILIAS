...
class UnboundGlobalProvider extends SeparatedUnboundProvider
{
    ...
    public function buildComponentsOf($component_type, Entity $entity)
    {
        if ($component_type === MailContext::class) {
            return [
                new Mailing\MailContextBookingModalities($entity, $this->owner())
            ];
        }
    ...
    public static function createGlobalProvider()
    {
        ...
        if (!$provider) {
            ...
            $provider_db->createSeparatedUnboundProvider(
                ...
                'Customizing/global/plugins/Services/Repository/RepositoryObject/BookingModalities/classes/UnboundGlobalProvider.php'
            );
        }
    }
