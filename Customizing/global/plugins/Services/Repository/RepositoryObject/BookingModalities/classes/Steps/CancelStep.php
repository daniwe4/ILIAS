...
abstract class CancelStep
{
     ...
     * @return bool
     */
    protected function canCancel(\ilDateTime $crs_start = null)
    {
        $storno_modus = $this->owner->getStorno()->getModus();
        $correct_mode = $storno_modus == $this->requiredCancelMode();

        if ($crs_start === null && $correct_mode) {
            return true;
        } elseif ($crs_start === null && !$correct_mode) {
            return false;
        }
        ...
        $today = date("Y-m-d");
        if ($correct_mode
            && $today <= $storno_end_date->get(IL_CAL_DATE)) {
            return true;
        }
