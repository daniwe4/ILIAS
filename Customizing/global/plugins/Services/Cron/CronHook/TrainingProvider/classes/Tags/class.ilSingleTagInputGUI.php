...
* @version $Id$
*/
class ilSingleTagInputGUI extends ilTextInputGUI
{
    ...
    public function __construct($plugin_object, $a_title = "", $a_postvar = "")
    {
            ...
            foreach ($clean_values as $key => $value) {
                if ($value["name"] === null || strlen($value["name"]) < self::MINIMUM_LENGTH_NAME) {
                    ...
                    if (!$erro_messages["message_name_short"]) {
                        $erro_messages["message_name_short"] = sprintf($this->plugin_object->txt("error_name_to_short"), self::MINIMUM_LENGTH_NAME);
                    }
                }
                ...
                if ($value["color"] === null || !$this->checkColorValid($value["color"])) {
                    ...
                    if (!$erro_messages["message_color_code_invalid"]) {
                        $erro_messages["message_color_code_invalid"] = $this->plugin_object->txt("error_color_code_invalid");
                    }
                }
            }
    ...
    */
    ...
    {
        $new_tpl = new ilTemplate("tpl.single_tag_input.html", true, true, $this->plugin_object->getDirectory());

        $new_tpl->setVariable("NAME_TITLE", $this->plugin_object->txt("tag_name"));
        $new_tpl->setVariable("COLOR_TITLE", $this->plugin_object->txt("tag_color"));
        $new_tpl->setVariable("COLOR_EXAMPEL_TITLE", $this->plugin_object->txt("tag_color_exampel"));
        $new_tpl->setVariable("ALLOCS_TITLE", $this->plugin_object->txt("tag_count_allocs"));
        $new_tpl->setVariable("ACTIONS_TITLE", $this->plugin_object->txt("tag_actions"));
        ...
        foreach ($values as $key => $value) {
            ...
            if ($value["color"] != "" && $value["color"] !== null && $this->checkColorValid($value["color"])) {
                ...
                $new_tpl->setVariable("IMAGE", $this->plugin_object->getDirectory() . "/templates/images/icon_not_ok.svg");
                $new_tpl->parseCurrentBlock();
            } else {
        ...
        $this->g_tpl->addJavascript($this->plugin_object->getDirectory() . "/templates/js/ServiceTagMulti.js");
