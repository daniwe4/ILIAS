...
SELECT
	...
	oref.ref_id AS xbkm_ref_id,
	xcps_tpl_crs.crs_ref_id AS is_template,
	xbkm_waitinglist.cancellation,
	hhd_crs.begin_date,
	acwaiting_log.state AS state
FROM
	tree
...
JOIN object_data od ON od.obj_id = oref.obj_id
	AND od.type = "xbkm"
JOIN xbkm_waitinglist ON xbkm_waitinglist.obj_id = oref.obj_id
	AND xbkm_waitinglist.modus != "no_waitinglist"
	AND xbkm_waitinglist.cancellation IS NOT NULL
LEFT JOIN xcps_tpl_crs ON xcps_tpl_crs.crs_ref_id = object_reference.ref_id
LEFT JOIN acwaiting_log ON acwaiting_log.crs_ref_id = object_reference.ref_id
WHERE
	tree.path BETWEEN "1" AND "1.Z"
		AND tree.tree = 1
		AND object_data.type = "crs"
		AND object_reference.deleted IS NULL
		AND hhd_crs.begin_date >= CURDATE()
HAVING is_template IS NULL AND state IS NULL
ORDER BY object_reference.ref_id, xbkm_waitinglist.cancellation
EOT;
        ...
        while ($row = $this->db->fetchAssoc($res)) {
            ...
            if (!array_key_exists($crs_ref_id, $ret)) {
                $crs_data = new CourseData(
                    $crs_ref_id,
                    new \DateTime($row["begin_date"]),
                    (int) $row["xbkm_ref_id"],
                    (int) $row["cancellation"]
                );
                $ret[$crs_ref_id] = $crs_data;
            }
        }
