...
class ilExportZipBuiltObjective extends ilSetupObjective
{
    ...
    {
        $export_hook_path = $this->config->getExportHooksPath();
        $path = null;
        if ('' != $export_hook_path) {
            $path = $export_hook_path;
        }
        $dumper = new MysqlExecDumper($path);
    ...
    {
        $export_hooks_path = $this->config->getExportHooksPath();
        if ('' != $export_hooks_path && !is_readable($export_hooks_path)) {
            throw new Exception("Export hooks file '$export_hooks_path' is not readable.");
        }
