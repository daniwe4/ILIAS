...
class SqlQueryInterpreterTest extends TestCase
{
    ...
    public function test_field()
    {
        ...
        $this->assertRegExp(
            '#\\s*SUM\\(\\s*MIN\\(\\s*a\\.f1\\s*\\)\\s*/\\s*MAX\\(\\s*a\\.f2\\s*\\)\\s*\\)\\s*#',
            $i->_interpretField(
                $this->tf->sum(
                    'sum',
                    $this->tf->quot(
                        'quot',
                        $this->tf->min('min', $table->field('f1')),
        ...
        $this->assertRegExp('#IF\\(\\s*`a`\\.`f1`\\s*=\\s*`a`\\.`f2`\s*,\\s*a\\.f1\\s*,\\s*a\\.f2\\s*\\)#', $i->_interpretField($this->tf->ifThenElse('foo', $f1->EQ($f2), $f1, $f2)));
    }
