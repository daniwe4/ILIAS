...
class FilterTypeTest extends TestCase
{
    ...
    public function class_provider()
    {
        ...
        return array( array("\\stdClass", new \stdClass(), true)
    ...
    {
        return array( array(function ($f) {
            return $f->tuple($f->cls("\\stdClass"));
        }, new \stdClass(), false)
            , array(function ($f) {
                return $f->tuple($f->string());
            }, "", false)
            , array(function ($f) {
                return $f->tuple($f->string());
            }, null, false)
            , array(function ($f) {
                return $f->tuple($f->int());
            }, 42, false)
            , array(function ($f) {
                return $f->tuple($f->int());
            }, array(), false)
    ...
    {
        return array( array(function ($f) {
            return $f->dict(array("cls" => $f->cls("\\stdClass")));
        }, array("cls2" => new \stdClass()), false)
            , array(function ($f) {
                return $f->dict(array("cls" => $f->cls("\\stdClass")));
            }, array("cls" => new \stdClass()), true)
            , array(function ($f) {
                return $f->dict(array("str" => $f->string()));
            }, null, false)
            , array(function ($f) {
                return $f->dict(array("int" => $f->int()));
            }, array(42), false)
            , array(function ($f) {
                return $f->dict(array("int" => $f->int()));
            }, array("int" => 42), true)
            , array(function ($f) {
                return $f->dict(array($f->int()));
            }, array(1), true)
            , array(function ($f) {
                return $f->dict(array("int" => $f->int()));
            }, array("int" => "1"), false)
            , array(function ($f) {
                return $f->dict(array("int1" => $f->int(), "int2" => $f->int()));
            }, array("int2" => 1, "int1" => 2), true)
            , array(function ($f) {
                return $f->dict(array($f->int(), $f->int()));
            }, array(1,"2"), false)
            );
    }
    ...
    {
        return array( array(function ($f) {
            return $f->either($f->int(), $f->string());
        }, 1, true)
            , array(function ($f) {
                return $f->either($f->int(), $f->string());
            }, "2", true)
    ...
    {
        return array( array(function ($f) {
            return $f->option($f->cls("\\stdClass"));
        }, array(0, new \stdClass()), true)
            , array(function ($f) {
                return $f->option($f->cls("\\stdClass"));
            }, array(1, new \stdClass()), false)
            , array(function ($f) {
                return $f->option($f->cls("\\stdClass"));
            }, new \stdClass(), false)
            , array(function ($f) {
                return $f->option($f->int(), $f->string());
            }, array(0, 1), true)
            , array(function ($f) {
                return $f->option($f->int(), $f->string());
            }, array(1, "23"), true)
    ...
    public function unflatten_provider()
    {
        return array( array(function ($f) {
            ...
            , array(function ($f) {
                return $f->tuple($f->string(), $f->int());
            }, array("a", 1), array("a", 1))
            , array(function ($f) {
                return $f->tuple($f->string(), $f->int());
            }, array("a", 1), array("a", 1))
            , array(function ($f) {
                return $f->tuple($f->string(), $f->tuple($f->int(), $f->int()));
            }, array("a", 1, 2), array("a", array(1,2)))
            , array(function ($f) {
                return $f->option($f->string(), $f->int());
            }, array(0, "1"), array(0, "1"))
            , array(function ($f) {
                return $f->option($f->string(), $f->int());
            }, array(1, 2), array(1, 2))
            , array(function ($f) {
                return $f->dict(array("str" => $f->string(), "tup" => $f->tuple($f->string(), $f->int())), true);
            }, array("a", "b", 1), array("str" => "a", "tup" => array("b", 1)))
            );
    }
}
